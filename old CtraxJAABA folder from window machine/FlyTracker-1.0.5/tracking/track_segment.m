            ...
            for fg_idx = 1:n_fgs
                ...
                for b = 1:numel(bods)
                    ...
                    if trks_matched
                        ...
                        rot_vec = [cos(ori) -sin(ori)];
                        vector = rot_vec;
                       % CHANGED BY ANNIKA JUNE 2018
                       %to deal with orientation errors
                     %  if flipped == 0
                      %    thresh_scale_factor = 1;
                      % elseif flipped == 1
                          % thresh_scale_factor = .5;
                           %disp('setting threshold scale factor to');
                           %disp(thresh_scale_factor);
                     %  end
                       
                        % disambiguate using velocity or previous frame  
                        thresh_scale_factor = .5;
                        if vel > params.vel_thresh*thresh_scale_factor && numel(bods) == 1 %ANNIKA CHANGED *.5 to *thresh_scale_factor
                            % use velocity
                            vector = velocities{seq_id}(fr_idx,2:3);
                            vector = vector / norm(vector);
                            %LINE ADDED BY ANNIKA
                            %change flipped between 0 and 1 each time the
                            %if condition is met (meaning the orientation
                            %gets
                            %flipped)
                           % disp('the orientation was flipped in frame number');
                          %  disp(i);
                          %  disp('setting flipped parameter to');
                           % flipped = flipped + (-1)^flipped;
                           % disp(flipped);
                            %END CHANGES ANNIKA
                        elseif i>1
                            % use previous orientation
                    ...
                    %ANNIKA CHANGES
                    %commented out because changes introduced severe errors
