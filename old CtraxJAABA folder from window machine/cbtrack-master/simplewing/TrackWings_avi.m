function [trx,perframedata,outtrxfile,info] = TrackWings_avi(expdir,varargin)
...
nflies = numel(trx);
trxfiledata.trx = trx;
%changed by Annika Rings 2018 to fix error with arena parameters
for fly = 1:nflies
    trxfiledata.trx(fly).arena.x = read_ann(annfile,'arena_center_x');
    trxfiledata.trx(fly).arena.y = read_ann(annfile,'arena_center_y');
    trxfiledata.trx(fly).arena.r = read_ann(annfile,'arena_radius');
end
%end changes
