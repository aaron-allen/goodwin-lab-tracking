...
id_new = ids.id_new;
id_old = ids.id_old;
chambers = ids.chambers;
for i = 1:numel(id_old)
    data_new(id_new(i), :, :) = data(id_old(i), :, :);
end

trk.data = deal(data_new);
numchambers = max(chambers);
flies_per_chamber = max(cellfun(@(c)size(c,2), trk.flies_in_chamber));
all_flyids = [1:numchambers*flies_per_chamber];
flyarr = transpose(reshape(all_flyids,[flies_per_chamber, numchambers]));
flies_in_chamber = transpose(num2cell(flyarr,2));
